...
pub const CodeGenerator = struct {
    ...
    fn generateRuntimeHeader(self: *Self) !void {
        ...
        // POSIX feature test macro (must come before any includes)
        try self.writer.writeLine("#ifndef _WIN32");
        try self.writer.writeLine("#define _POSIX_C_SOURCE 199309L");
        ...
        // POSIX includes for terminal/timing builtins
        try self.writer.writeLine("#ifndef _WIN32");
        try self.writer.writeLine("#include <time.h>");
        try self.writer.writeLine("#include <poll.h>");
        try self.writer.writeLine("#include <sys/ioctl.h>");
        ...
        try self.writer.writeLine("static char** _dm_str_pool = NULL;");
        try self.writer.writeLine("static int64_t _dm_str_pool_len = 0;");
        try self.writer.writeLine("static int64_t _dm_str_pool_cap = 0;");
        try self.writer.writeLine("static inline void _dm_str_track(char* p) {");
        self.writer.indent();
        try self.writer.writeLine("if (_dm_str_pool_len >= _dm_str_pool_cap) {");
        self.writer.indent();
        try self.writer.writeLine("_dm_str_pool_cap = _dm_str_pool_cap ? _dm_str_pool_cap * 2 : 4096;");
        try self.writer.writeLine("_dm_str_pool = (char**)realloc(_dm_str_pool, (size_t)_dm_str_pool_cap * sizeof(char*));");
        self.writer.dedent();
        try self.writer.writeLine("}");
        try self.writer.writeLine("_dm_str_pool[_dm_str_pool_len++] = p;");
        self.writer.dedent();
        try self.writer.writeLine("}");
        try self.writer.writeLine("static inline int64_t dm_mem_mark(void) { return _dm_str_pool_len; }");
        try self.writer.writeLine("static inline void dm_mem_sweep(int64_t mark) {");
        self.writer.indent();
        try self.writer.writeLine("for (int64_t i = mark; i < _dm_str_pool_len; i++) free(_dm_str_pool[i]);");
        try self.writer.writeLine("_dm_str_pool_len = mark;");
        self.writer.dedent();
        try self.writer.writeLine("}");
        try self.writer.blankLine();

        // String concatenation (tracked)
        ...
        try self.writer.writeLine("_dm_str_track(result);");
        ...
        // Float to string conversion (tracked)
        ...
        try self.writer.writeLine("static inline int64_t dm_read_key(void) {");
        self.writer.indent();
        try self.writer.writeLine("int c = getchar();");
        try self.writer.writeLine("return (c == EOF) ? -1 : (int64_t)c;");
        self.writer.dedent();
        try self.writer.writeLine("}");
        try self.writer.blankLine();

        // Read single key from stdin (non-blocking, returns -1 if no input)
        try self.writer.writeLine("static inline int64_t dm_read_key_nb(void) {");
        self.writer.indent();
        try self.writer.writeLine("struct pollfd pfd = { .fd = 0, .events = POLLIN };");
        try self.writer.writeLine("if (poll(&pfd, 1, 0) > 0) {");
        self.writer.indent();
        try self.writer.writeLine("int c = getchar();");
        try self.writer.writeLine("return (c == EOF) ? -1 : (int64_t)c;");
        self.writer.dedent();
        ...
        try self.writer.writeLine("static inline dm_string dm_chr(int64_t n) {");
        self.writer.indent();
        try self.writer.writeLine("char* buf = (char*)malloc(2);");
        try self.writer.writeLine("buf[0] = (char)n;");
        try self.writer.writeLine("buf[1] = '\\0';");
        try self.writer.writeLine("_dm_str_track(buf);");
        try self.writer.writeLine("return (dm_string){ .data = buf, .len = 1, .capacity = 1 };");
        self.writer.dedent();
        ...
        try self.writer.writeLine("static inline void dm_sleep_ms(int64_t ms) {");
        self.writer.indent();
        try self.writer.writeLine("struct timespec ts = { .tv_sec = ms / 1000, .tv_nsec = (ms % 1000) * 1000000 };");
        try self.writer.writeLine("nanosleep(&ts, NULL);");
        self.writer.dedent();
        try self.writer.writeLine("}");
        try self.writer.blankLine();

        // Get terminal width
        try self.writer.writeLine("static inline int64_t dm_term_width(void) {");
        self.writer.indent();
        try self.writer.writeLine("struct winsize ws;");
        try self.writer.writeLine("if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == 0 && ws.ws_col > 0) return (int64_t)ws.ws_col;");
        try self.writer.writeLine("return 80;");
        self.writer.dedent();
        try self.writer.writeLine("}");
        try self.writer.blankLine();

        // Get terminal height
        try self.writer.writeLine("static inline int64_t dm_term_height(void) {");
        self.writer.indent();
        try self.writer.writeLine("struct winsize ws;");
        try self.writer.writeLine("if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == 0 && ws.ws_row > 0) return (int64_t)ws.ws_row;");
        try self.writer.writeLine("return 24;");
        self.writer.dedent();
        try self.writer.writeLine("}");
        try self.writer.blankLine();

        // Get current time in milliseconds (monotonic clock)
        try self.writer.writeLine("static inline int64_t dm_time_ms(void) {");
        self.writer.indent();
        try self.writer.writeLine("struct timespec ts;");
        try self.writer.writeLine("clock_gettime(CLOCK_MONOTONIC, &ts);");
        try self.writer.writeLine("return (int64_t)ts.tv_sec * 1000 + (int64_t)ts.tv_nsec / 1000000;");
        self.writer.dedent();
        try self.writer.writeLine("}");
        try self.writer.blankLine();
    ...
    fn inferCTypeFromExpr(self: *Self, expr: *Expr) []const u8 {
        return switch (expr.kind) {
            ...
            .function_call => |call| blk: {
                ...
                if (call.function.kind == .identifier) {
                    ...
                    if (std.mem.eql(u8, name, "len") or
                        ...
                        std.mem.eql(u8, name, "mutex_new") or
                        std.mem.eql(u8, name, "read_key") or
                        std.mem.eql(u8, name, "read_key_nb") or
                        std.mem.eql(u8, name, "term_width") or
                        std.mem.eql(u8, name, "term_height") or
                        std.mem.eql(u8, name, "time_ms")) break :blk "int64_t";
                    ...
                    if (std.mem.eql(u8, name, "to_string") or
                        ...
                        std.mem.eql(u8, name, "tcp_read") or
                        std.mem.eql(u8, name, "chr")) break :blk "dm_string";
            ...
            .pipeline => |pipe| blk: {
                // Pipeline x |> f has the type of f(x). Infer from the right side function.
                const func_name: ?[]const u8 = switch (pipe.right.kind) {
                    .function_call => |fc| if (fc.function.kind == .identifier) fc.function.kind.identifier.name else null,
    ...
    fn generateBuiltinCall(self: *Self, name: []const u8, call: *FunctionCall) !bool {
        if (std.mem.eql(u8, name, "print")) {
            ...
            return true;
        } else if (std.mem.eql(u8, name, "chr")) {
            if (call.args.len > 0) {
                try self.writer.write("dm_chr(");
                try self.generateExpr(call.args[0].value);
                try self.writer.write(")");
            }
            return true;
            ...
            return true;
        } else if (std.mem.eql(u8, name, "read_key")) {
            try self.writer.write("dm_read_key()");
            return true;
        } else if (std.mem.eql(u8, name, "read_key_nb")) {
            try self.writer.write("dm_read_key_nb()");
            return true;
        } else if (std.mem.eql(u8, name, "term_width")) {
            try self.writer.write("dm_term_width()");
            return true;
        } else if (std.mem.eql(u8, name, "term_height")) {
            try self.writer.write("dm_term_height()");
            return true;
        } else if (std.mem.eql(u8, name, "time_ms")) {
            try self.writer.write("dm_time_ms()");
            return true;
        } else if (std.mem.eql(u8, name, "sleep_ms")) {
            if (call.args.len > 0) {
                try self.writer.write("dm_sleep_ms(");
                try self.generateExpr(call.args[0].value);
                try self.writer.write(")");
            }
            return true;
        } else if (std.mem.eql(u8, name, "flush")) {
            try self.writer.write("fflush(stdout)");
            return true;
        } else if (std.mem.eql(u8, name, "mem_mark")) {
            try self.writer.write("dm_mem_mark()");
            return true;
        } else if (std.mem.eql(u8, name, "mem_sweep")) {
            if (call.args.len > 0) {
                try self.writer.write("dm_mem_sweep(");
                try self.generateExpr(call.args[0].value);
                try self.writer.write(")");
            }
            return true;
    ...
    fn evalComptimeFunctionCall(self: *Self, call: *const FunctionCall, env: *ComptimeEnv) ?ComptimeValue {
        ...
        const impure_builtins = [_][]const u8{
            ...
            "exit",      "system",     "panic",        "read_key",
            "read_key_nb", "sleep_ms", "term_width",   "term_height",
            "time_ms",   "flush",      "mem_mark",    "mem_sweep",
        };
